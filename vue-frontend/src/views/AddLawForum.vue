<template>
    <div class="form, container">
         <div class="form-section">
            <span>Gesetz</span>
            <input type="text" value="PART.CAMO" placeholder="PART.CAMO" v-model="law.l_gesetz" disabled/>
        ...
        <div class="form-section">
            ...
            <span v-if="v$.law.l_gueltigAb.$error">{{ v$.law.l_gueltigAb.$errors[0].$message }}</span>
        </div>

        <button type="button" class="btn btn-primary" v-on:click="saveLaw()">Speichern</button>
        <RouterView/>
    </div>
</template>
...
export default {
    ...
    methods: {
        async saveLaw() {
            this.v$.$validate()
            if (!this.v$.$error) {
                const response = await axios.post("http://localhost:8080/api/laws/", this.law);
